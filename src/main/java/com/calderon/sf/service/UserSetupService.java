...
public class UserSetupService {
    ...
    private List<FinanceAccount> defaultAccounts(User user){
        ...
        accounts.add(new FinanceAccount().accountNumber("1").accountStatus(AccountStatus.ACTIVE).name("Wallet").description("Wallet").isCreditCard(false).user(user));
        ...
        accounts.add(new FinanceAccount().accountNumber("************0003").accountStatus(AccountStatus.ACTIVE).name("Credit Card").description("Credit Card").isCreditCard(true).user(user));
        return accounts;
    }
